...
4. Коэффициент вариации: от {cv_min:.0f}% ({cv_min_col}) до {cv_max:.0f}% ({cv_max_col}).
   Требуется StandardScaler, чтобы привести все признаки к единому масштабу.
...
print(f"""
1. Тренд: падает (изменение на {trend_change:.2f}°C).
   Интенсивность: сильная.

2. Сезонность: период ~{period} сек, амплитуда низкая ({seasonal_amp:.2f}°C).
   Связана с циклами включения/выключения насоса.

3. SNR = {snr:.2f} дБ — отлично.
   Шум практически незаметен.

4. Распределение остатков: близко к нормальному.
   Шум случаен, декомпозиция адекватна.

5. Фильтрация: не требуется.
""" + "\n" + "=" * 50 + "\nАНАЛИЗ ЗАВЕРШЁН\n" + "=" * 50)
