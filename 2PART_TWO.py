...
print("=" * 50+
      ...
      f"\nChangepoint: {int(df_exp['changepoint'].sum())} "
...
fig.suptitle('Рисунок 14. Визуализация всех каналов - эксперимент 1',
...
print("\n" + "-" * 50+
      "\nВЫВОДЫ:\n"+
      "-" * 50+ f"""
1. Визуализированы 8 каналов эксперимента 1.
2. Аномалий (anomaly=1): {int(df_exp['anomaly'].sum())} точек.
3. Аномалии совпадают с резкими скачками на графиках.
4. Каналы взаимосвязаны, видны циклы работы насоса.
5. Разрывов и пропусков нет.
""")
# ============================================
...
# 1. ПОЛНАЯ ТАБЛИЦА
print("\nОписательные статистики измерительных каналов\n"+
...
for col in data_cols:
    ...
    if abs(mean_v - median_v) > 0.1 * abs(mean_v):
        asym_cols.append(col)

# Разброс
cv_max_col = max(data_cols, key=lambda c: df[c].std()/df[c].mean()*100 if df[c].mean() != 0 else 0)
cv_max = df[cv_max_col].std()/df[cv_max_col].mean()*100 if df[cv_max_col].mean() != 0 else 0
cv_min_col = min(data_cols, key=lambda c: df[c].std()/df[c].mean()*100 if df[c].mean() != 0 else 0)
cv_min = df[cv_min_col].std()/df[cv_min_col].mean()*100 if df[cv_min_col].mean() != 0 else 0

print(f"""
1. Всего {len(df)} строк, 8 каналов.

2. Частота: {freq:.1f} Гц (1 измерение в секунду).

3. Сильная асимметрия: {', '.join(asym_cols[:3])}...
   Причина: насос большую часть времени выключен (значения ~0).

4. Коэффициент вариации: от {cv_min:.0f}% ({cv_min_col}) до {cv_max:.0f}% ({cv_max_col}).
   Требуется StandardScaler.

5. Каналов с нулевым std нет.
""")
# ============================================
...
print("\n" + "-" * 50 + "\nВЫВОДЫ:\n" + "-" * 50+
      f"""\n
1. Пропуски в измерительных каналах: {total_missing} из {len(df) * len(data_cols)}.
   Данные полные — обработка пропусков не требуется.

2. Выбросы (правило 3σ):
   - Accelerometer1RMS: 1253 (2.67%) — пиковые вибрации при включении.
   - Accelerometer2RMS: 1227 (2.62%) — аналогично.
   - Current, Pressure, Voltage: <1% — редкие выбросы.
   - Temperature, Thermocouple, Volume Flow Rate: 0% — стабильные показатели.

3. Переходные режимы насоса это не ошибки, а целевые события.
   Удалять выбросы НЕЛЬЗЯ — это искомые аномалии.

4. Boxplot подтверждает концентрацию значений вблизи нуля (насос выключен)
   и наличие выбросов в моменты активной работы.

5. Для ML-моделей потребуется масштабирование (StandardScaler).
""")
# ============================================
# 5. Анализ диапазонов значений (SKAB)
# ============================================
print("=" * 50 + "\n5. АНАЛИЗ ДИАПАЗОНОВ ЗНАЧЕНИЙ\n" + "=" * 50 +
      "\n\n1. ДИАПАЗОНЫ ЗНАЧЕНИЙ ПО КАНАЛАМ:\n" +
...
ratio = ranges[max_range_col] / ranges[min_range_col]

print(f"""
1. Каналы измеряются в разных единицах (g, А, бар, °C, В, л/мин).

2. Максимальный размах: {max_range_col} ({ranges[max_range_col]:.1f}).
   Минимальный размах: {min_range_col} ({ranges[min_range_col]:.3f}).
   Отношение: {ratio:.0f}:1 — масштабы сильно различаются.

3. Boxplot подтверждает:
   - Разные диапазоны и разброс по каналам.
   - Выбросы есть в Accelerometer, Current, Pressure.

4. Требуется стандартизация (StandardScaler) перед ML-моделями.
   Нормализация не требуется для деревьев решений.
""")
# ============================================
...
print("\n" + "-" * 50 +
      "\nВЫВОДЫ:\n"
      + "-" * 50 + f"""
1. Сильные положительные корреляции (|r| > 0.7): {len(strong_pairs)} пар.
   {"Каналы дублируют информацию друг друга." if len(strong_pairs) >= 3 else "Сильной мультиколлинеарности нет."}

2. Отрицательные корреляции: {"обнаружены" if any(r < -0.7 for _, _, r in strong_pairs) else "сильных не обнаружено"}.

3. Каналы с уникальной информацией (слабо связаны с остальными):
   Temperature, Thermocouple — могут нести независимую информацию.

4. Для ML-моделей:
   - Сильно коррелирующие каналы можно объединить или отбросить.
   - Temperature и Thermocouple стоит сохранить как независимые признаки.
""")
...
# Анализ тренда
trend_start = trend.iloc[:min_len].values[0]
trend_end = trend.iloc[:min_len].values[-1]
trend_change = trend_end - trend_start
trend_direction = "растёт" if trend_change > 0.5 else ("падает" if trend_change < -0.5 else "стабилен")

# Анализ сезонности
seasonal_amp = np.max(seasonal.iloc[:min_len]) - np.min(seasonal.iloc[:min_len])
if seasonal_amp > 2:
    season_str = "высокая"
elif seasonal_amp > 0.5:
    season_str = "средняя"
else:
    season_str = "низкая"

print(f"""
1. Тренд: {trend_direction} (изменение на {trend_change:.2f}°C).
   Интенсивность: {"сильный" if abs(trend_change) > 2 else "слабый"}.

2. Сезонность: период ~{period} сек, амплитуда {season_str} ({seasonal_amp:.2f}°C).
   Связана с циклами включения/выключения насоса.

3. SNR = {snr:.2f} дБ — {snr_quality}.
   {"Шум практически незаметен." if snr > 20 else "Сигнал доминирует над шумом." if snr > 10 else "Требуется фильтрация."}

4. Распределение остатков: {"близко к нормальному" if abs(noise.skew()) < 0.5 else "асимметричное"}.
   {"Шум случаен, декомпозиция адекватна." if abs(noise.skew()) < 0.5 else "Возможна систематическая ошибка измерений."}

5. Фильтрация: {"не требуется" if snr > 10 else "рекомендуется медианный фильтр"}.
""" + "\n" + "=" * 50 + "\nАНАЛИЗ ЗАВЕРШЁН\n" + "=" * 50)
